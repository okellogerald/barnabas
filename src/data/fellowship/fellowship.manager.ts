...
export class FellowshipManager {
   ...
   */
  private constructor(repo: FellowshipRepository, permissionsManager: AuthorizationManager) {
    this._repo = repo;
  ...
  public async getFellowshipsCount(): Promise<number> {
    ...
    if (!this._permissionsManager.canPerformAction(Actions.FELLOWSHIP_FIND_ALL)) {
   ...
   */
  public async getAll(queryParams?: FellowshipQueryParams): Promise<Fellowship[]> {
    if (!this._permissionsManager.canPerformAction(Actions.FELLOWSHIP_FIND_ALL)) {
      throw PermissionError.fromAction(Actions.FELLOWSHIP_FIND_ALL);
    }
   ...
   * @throws {Error} If there is an error retrieving fellowships from the repository.
   */
  public async getFellowships(queryParams?: FellowshipQueryParams): Promise<GetFellowshipsResponse> {
    if (!this._permissionsManager.canPerformAction(Actions.FELLOWSHIP_FIND_ALL)) {
      throw PermissionError.fromAction(Actions.FELLOWSHIP_FIND_ALL);
    }
   ...
   */
  public async createFellowship(fellowship: CreateFellowshipDTO): Promise<Fellowship> {
    if (!this._permissionsManager.canPerformAction(Actions.FELLOWSHIP_CREATE)) {
    ...
    try {
      ...
      if (!dto) {
        throw new Error("Fellowship not found immediately after creation!");
      }
   ...
   */
  public async getFellowshipById(fellowshipId: string): Promise<Fellowship | undefined> {
    if (!this._permissionsManager.canPerformAction(Actions.FELLOWSHIP_FIND_BY_ID)) {
    ...
    try {
      ...
      console.error(`Error retrieving fellowship by ID (${fellowshipId}):`, error);
      throw error;
    }
  }
   ...
   */
  public async updateFellowship(fellowshipId: string, data: UpdateFellowshipDTO): Promise<Fellowship> {
    if (!this._permissionsManager.canPerformAction(Actions.FELLOWSHIP_UPDATE)) {
    ...
    try {
      ...
      console.error(`Error updating fellowship (${fellowshipId}):`, error);
      throw error;
    }
  }
  ...
  public async deleteFellowship(fellowshipId: string): Promise<void> {
    if (!this._permissionsManager.canPerformAction(Actions.FELLOWSHIP_DELETE_BY_ID)) {
    ...
    try {
      ...
      console.error(`Error deleting fellowship (${fellowshipId}):`, error);
      throw error;
    }
  }
   ...
   */
  public async getFellowshipsWithMemberCount(): Promise<GetFellowshipsResponse> {
    // Need both fellowship and member permissions for this operation
    if (!this._permissionsManager.canPerformAction(Actions.FELLOWSHIP_FIND_ALL)) {
      throw PermissionError.fromAction(Actions.FELLOWSHIP_FIND_ALL);
    }
    if (!this._permissionsManager.canPerformAction(Actions.MEMBER_FIND_ALL)) {
    ...
    try {
      ...
      const fellowships = await Promise.all(
        response.results.map(async (dto) => {
          ...
          try {
            ...
            const memberCount = await manager.getMembersCount(query);
            // Set the member count on the fellowship object
            fellowship.memberCount = memberCount;
          } catch (countError) {
            console.error(`Error getting member count for fellowship ${fellowship.id}:`, countError);
      ...
      console.error("Error retrieving fellowships with member count:", error);
      throw error;
    }
  }
}
