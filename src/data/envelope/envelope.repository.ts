...
import { EnvelopeAssignmentDTO, EnvelopeBlockDTO, EnvelopeDTO, EnvelopeHistoryDTO } from "./envelope.schema";
...
export class EnvelopeRepository extends BaseRepository<typeof envelopeContract> {
   ...
   */
  async getAll(queryBuilder: EnvelopeQueryBuilder): Promise<{ results: EnvelopeDTO[]; total: number }> {
    console.log(queryBuilder.build());
    const result = await this.client.getAll({
      query: queryBuilder.build(),
    });
    return this.handleResponse<{ results: EnvelopeDTO[]; total: number }>(result, 200);
  }
  ...
  async getCount(builder: EnvelopeQueryBuilder): Promise<number> {
    ...
    const countBuilder = EnvelopeQueryBuilder.is(builder) ? builder.clone().configureForCount() : builder;
   ...
   */
  async createBlock(data: EnvelopeBlockDTO): Promise<{ count: number; startNumber: number; endNumber: number }> {
    const result = await this.client.createBlock({ body: data });
    return this.handleResponse<{ count: number; startNumber: number; endNumber: number }>(result, 201);
  }

  /**
   * Delete a block of envelopes
   */
  async deleteBlock(data: EnvelopeBlockDTO): Promise<{ count: number; startNumber: number; endNumber: number }> {
    const result = await this.client.deleteBlock({ body: data });
    return this.handleResponse<{ count: number; startNumber: number; endNumber: number }>(result, 200);
  }

  /**
   * Assign an envelope to a member
   */
  async assign(envelopeId: string, data: EnvelopeAssignmentDTO): Promise<EnvelopeDTO> {
