...
import { userContract } from "./user.api-contract";
import { CreateUserDTO, UpdateUserDTO, UserDTO, UserQueryParams } from "./user.schema";
...
export class UserRepository extends BaseRepository<typeof userContract> {
   ...
   */
  async getAll(queryParams: UserQueryParams = UserRepository.defaultQueryParams): Promise<GetUsersResponse> {
    try {
  ...
  async getByActiveStatus(isActive: boolean): Promise<GetUsersResponse> {
    try {
      ...
      console.error(`Error in getByActiveStatus with status ${isActive}:`, error);
      throw error;
    }
  }
}
