import { CreateUserDTO, UpdateUserDTO, UserQueryParams, UserRepository } from "@/data/user";
import { PermissionError } from "@/lib/error";
import { User } from "@/models/user.model";
import { Actions, AuthorizationManager } from "../authorization";
...
export class UserManager {
   ...
   */
  private constructor(repo: UserRepository, permissionsManager: AuthorizationManager) {
    this._repo = repo;
  ...
  public static get instance(): UserManager {
    if (!UserManager._instance) {
      UserManager._instance = new UserManager(new UserRepository(), AuthorizationManager.getInstance());
    }
   ...
   */
  public async getUsersCount(queryParams?: UserQueryParams): Promise<number> {
    // Check if the user has permission to find users
    if (!this._permManager.canPerformAction(Actions.USER_FIND_ALL)) {
   ...
   */
  public async getUsers(queryParams?: UserQueryParams): Promise<GetUsersResponse> {
    if (!this._permManager.canPerformAction(Actions.USER_FIND_ALL)) {
      throw PermissionError.fromAction(Actions.USER_FIND_ALL);
    }
  ...
  public async createUser(user: CreateUserDTO): Promise<User> {
    if (!this._permManager.canPerformAction(Actions.USER_CREATE)) {
  ...
  public async getUserById(userId: string): Promise<User | undefined> {
    if (!this._permManager.canPerformAction(Actions.USER_FIND_BY_ID)) {
  ...
  public async updateUser(userId: string, data: UpdateUserDTO): Promise<User> {
    if (!this._permManager.canPerformAction(Actions.USER_UPDATE)) {
  ...
  public async deleteUser(userId: string): Promise<void> {
    if (!this._permManager.canPerformAction(Actions.USER_DELETE)) {
