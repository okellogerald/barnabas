...
export class VolunteerOpportunityRepository extends BaseRepository<typeof volunteerOpportunityContract> {
  constructor() {
   ...
   */
  async getAll(queryBuilder: VolunteerOpportunityQueryBuilder): Promise<VolunteerOpportunityDTO[]> {
    const query = queryBuilder.build();
    const result = await this.client.getAll({ query });
    const opportunities = this.handleResponse<VolunteerOpportunityDTO[]>(result, 200);
  ...
  async getPaginated(
    ...
    const opportunities = this.handleResponse<{ results: VolunteerOpportunityDTO[]; total: number }>(result, 200);
  ...
  async getCount(builder: VolunteerOpportunityQueryBuilder): Promise<number> {
    ...
    const countBuilder = VolunteerOpportunityQueryBuilder.is(builder) ? builder.clone().configureForCount() : builder;
   ...
   */
  async create(data: CreateVolunteerOpportunityDTO): Promise<VolunteerOpportunityDTO> {
   ...
   */
  async update(id: string, data: UpdateVolunteerOpportunityDTO): Promise<VolunteerOpportunityDTO> {
