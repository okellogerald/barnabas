...
export class RoleManager {
   ...
   */
  private constructor(repo: RoleRepository, permissionsManager: AuthorizationManager) {
    this._repo = repo;
  ...
  public static get instance(): RoleManager {
    if (!RoleManager._instance) {
      RoleManager._instance = new RoleManager(new RoleRepository(), AuthorizationManager.getInstance());
    }
   ...
   */
  public async getRolesCount(queryParams?: RoleQueryParams): Promise<number> {
   ...
   */
  public async getRoles(queryParams?: RoleQueryParams): Promise<GetRolesResponse> {
    if (!this._permissionsManager.canPerformAction(Actions.ROLE_FIND_ALL)) {
      throw PermissionError.fromAction(Actions.ROLE_FIND_ALL);
    }
  ...
  public async getRoleById(roleId: string): Promise<Role | undefined> {
    if (!this._permissionsManager.canPerformAction(Actions.ROLE_FIND_BY_ID)) {
    ...
    try {
      ...
      console.error(`Error retrieving role by ID (${roleId}):`, error);
      throw error;
    }
  }
  ...
  public async getRoleActions(roleId: string): Promise<RoleAction[]> {
    ...
    try {
      ...
      console.error(`Error retrieving actions for role (${roleId}):`, error);
      throw error;
    }
  }
}
