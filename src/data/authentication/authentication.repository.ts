...
import { authContract } from "./authentication.api-contract";
import { AuthResponse, CurrentUserResponse, LoginRequest } from "./authentication.schema";
...
export class AuthRepository extends BaseRepository<typeof authContract> {
  ...
  async login(credentials: LoginRequest): Promise<AuthResponse> {
    const result = await this.client.login({ body: credentials });
