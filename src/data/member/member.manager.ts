import { CreateMemberDTO, MemberRepository, UpdateMemberDTO } from "@/data/member";
...
export class MemberManager {
   ...
   */
  private constructor(repo: MemberRepository, permissionsManager: AuthorizationManager) {
    this._repo = repo;
  ...
  public static get instance(): MemberManager {
    if (!MemberManager._instance) {
      MemberManager._instance = new MemberManager(new MemberRepository(), AuthorizationManager.getInstance());
    }
   ...
   */
  public async getMembersCount(options?: Parameters<MemberRepository["getCount"]>[0]): Promise<number> {
   ...
   */
  public async getPaginatedMembers(options?: MemberQueryCriteria | MemberQueryBuilder): Promise<GetMembersResponse> {
    if (!this._permManager.canPerformAction(Actions.MEMBER_FIND_ALL)) {
      throw PermissionError.fromAction(Actions.MEMBER_FIND_ALL);
    }
   ...
   * @throws {Error} If there is an error retrieving members from the repository.
   */
  public async getMembers(options?: MemberQueryCriteria | MemberQueryBuilder): Promise<Member[]> {
    if (!this._permManager.canPerformAction(Actions.MEMBER_FIND_ALL)) {
      throw PermissionError.fromAction(Actions.MEMBER_FIND_ALL);
    }
  ...
  public async getMemberByID(memberId: string): Promise<Member | undefined> {
    ...
    try {
      ...
      console.error(`Error retrieving member by ID (${memberId}):`, error);
      throw error;
    }
  }
   ...
   */
  public async updateMember(memberId: string, data: UpdateMemberDTO): Promise<Member> {
