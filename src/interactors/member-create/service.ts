...
import { MemberCreateFormSchema, MemberCreateFormValues } from "./schemas/schemas.member";
...
export const memnberCreateService = {
  submitForm: async (formValues: MemberCreateFormValues) => {
    ...
    if (!result.success) {
      ...
      notifyUtils.error("Please complete all required fields with valid data");
      return;
    }

    const toastId = notifyUtils.showLoading(MEMBER_NOTIFICATIONS.CREATE.LOADING);
    try {
